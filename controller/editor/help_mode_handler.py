...
class HelpModeHandler(BaseModeHandler):
    ...
    def handle_mode(self, key: str) -> bool:
        if key == 'ESC':
            ...
            self._tmp_filename = self._model.get_tmp_filename()
            self._model.load_file(filename=self._tmp_filename)
            pos = self._model.get_cursor_position()
            if pos[1] > 0:
                self._move_cursor(column=pos[1] - 1)
            return True
